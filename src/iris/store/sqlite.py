...
from ..session._tool_events import prepare_tool_event_append
...
class SQLiteStore:
    ...
    def append_tool_event(
        ...
        """按 event 内稳定 ID 幂等追加工具结果事件。"""
        try:
            with sqlite3.connect(self.path) as connection:
                row = connection.execute(
                    "SELECT tool_events_json FROM sessions WHERE session_id = ?",
                    (session_id,),
                ).fetchone()
                events = cast(list[dict[str, object]], json.loads(row[0])) if row else []
                payload = prepare_tool_event_append(events, event)
                if payload is None:
                    return
                ...
                connection.execute(
                    """
                    INSERT INTO sessions (session_id, tool_events_json, updated_at)
                    VALUES (?, ?, ?)
                    ON CONFLICT(session_id) DO UPDATE SET
                        tool_events_json = excluded.tool_events_json,
                        updated_at = excluded.updated_at
                    """,
                    (session_id, _dump_json(events), updated_at),
                )
        except sqlite3.Error as exc:
            raise IrisSessionError(
                "SQLite session 工具事件写入失败",
                path=str(self.path),
