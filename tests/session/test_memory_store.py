...
def test_memory_store_appends_same_idempotent_event_once() -> None:
    store = InMemorySessionStore()
    event = {"tool_name": "write_file", "result": {"path": "notes.txt"}}

    store.append_tool_event_once("session_1", "tool_result:run_1:call_1", event)
    store.append_tool_event_once(
        "session_1",
...
def test_memory_store_rejects_different_payload_for_existing_event_id() -> None:
    store = InMemorySessionStore()
    store.append_tool_event_once("session_1", "tool_result:run_1:call_1", {"status": "ok"})

    with pytest.raises(IrisSessionError):
        store.append_tool_event_once("session_1", "tool_result:run_1:call_1", {"status": "error"})


def test_memory_store_preserves_non_idempotent_append_behavior() -> None:
    store = InMemorySessionStore()
    event = {"tool_name": "write_file", "status": "ok"}

    store.append_tool_event("session_1", event)
    store.append_tool_event("session_1", event)

    assert store.load_tool_events("session_1") == [event, event]
...
def test_memory_store_rejects_conflicting_or_non_json_idempotent_events() -> None:
    ...
    with pytest.raises(IrisSessionError):
        store.append_tool_event_once("session_1", "tool_result:run_1:call_1", {"not_json": {"set"}})
